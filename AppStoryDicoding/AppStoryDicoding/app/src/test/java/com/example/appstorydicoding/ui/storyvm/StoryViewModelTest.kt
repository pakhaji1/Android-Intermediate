package com.example.appstorydicoding.ui.storyvm
...
class StoryViewModelTest{
    ...
    fun `when Get Story Should Not Null and Return Data`() = runTest {
        ...
        val actualStory: PagingData<ListStory> = storyViewModel.getStory(TOKEN).getOrAwaitValue()
    ...
    fun `when Get Quote Empty Should Return No Data`() = runTest {
        ...
        Mockito.`when`(storyRepository.getStories(TOKEN)).thenReturn(expectedQuote)
        val storyViewModel = StoryViewModel(storyRepository)
        val actualQuote: PagingData<ListStory> = storyViewModel.getStory(TOKEN).getOrAwaitValue()
        val differ = AsyncPagingDataDiffer(
            diffCallback = StoryAdapter.DIFF_CALLBACK,
            updateCallback = noopListUpdateCallback,
            workerDispatcher = Dispatchers.Main,
